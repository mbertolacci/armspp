...
int test(ENVELOPE *env, POINT *p, Function lpdf, METROPOLIS *metrop);

int update(ENVELOPE *env, POINT *p, Function lpdf, METROPOLIS *metrop);
...
double perfunc(Function lpdf, ENVELOPE *env, double x);
...
SEXP arms(
  ...
  int err = arms_internal(
    xinit.begin(), xinit.size(), xl, xr, lpdf, convex, npoint, dometrop, xprev,
    xsamp.begin(), nsamp, &neval
  );
  if (err > 0) {
    Rcpp::stop("Failed with error %d", err);
  }
...

{
  ...
  if ((p->x < xl) || (p->x > xr)) Rcpp::stop("arms error 1");
